// Queen of Hatred - a high-sec LCL specimen. She is a magical girl who only exists as long
// as there is villainy left to fight, so she runs on a VILLAINY meter instead of a qliphoth
// counter. Chaos in the facility feeds her; peace starves her. At zero she collapses into
// HYSTERIA - immobile, near-immune, and on a three-minute clock she can push back with
// Resist - and must choose whether to stay the hero or become the monster. Becoming the
// monster is the dragon breach, and the only way to contain that is to kill her.

// Her passive form's bolts MEND people rather than hurting them - that is the whole point of
// the meter's "heroism" half, and the dragon has no ranged attack at all, so this branch can
// simply be "is she still the hero?" rather than a faction test. Agents are still fair game
// for her melee, the beam and Arcana Beats. Whether the mend actually pays out is MendHuman's
// call - it only counts if there was something there to fix.
...
/mob/living/simple_animal/hostile/limbus_abno/hatred_queen
	...
	// Eight sweets. These are parent paths, so each covers its whole family - `cakeslice`
	// alone picks up every cake in the game, the strawberry charlotte and the blackberry-
	// and-strawberry ones included.
	diet_list = list(
	...
	/// The marker is a reward, not a utility: in the passive form she has to be either
	/// convinced the world needs her OR genuinely content to reach for it. EITHER bar clearing
	/// its threshold is enough. The dragon ignores both.
	...
	/// Her walking sprite. The source's "hatred" is a ONE-DIRECTION state - fine for an abno
	/// that stands in a cell, wrong for a player who walks around, because she never turns to
	/// face anything. "hatred_breach" is the same girl drawn in all four directions. Same
	/// reasoning as the Lunar Physician and Mountain, which both swap to a breach sprite for
	/// exactly this reason.
...
/mob/living/simple_animal/hostile/limbus_abno/hatred_queen/Initialize(mapload)
	...
	// The advanced medical HUD, the same one the health scanner glasses hand out
	// (clothing/glasses/hud.dm). She only earns villainy from mending someone who is
	// actually hurt, so she has to be able to SEE who is hurt without asking.
...
// The wand trails behind her. The ORDER is the whole trick, and it is why the source writes
// it this way: the wand is dropped on the turf she is standing on and THEN the parent walks
// her off it, so it ends up on the tile she just left. Moving it after ..() instead parks it
// permanently on top of her - and re-centring it on a timer (this used to also happen in
// Life()) fights OpenFire, which steps it out in front of her to fire past.
...
// Every villainy GAIN is announced in light green with a "(+villainy)" tag. The formatting
// lives here rather than at the call sites so it cannot drift between sources - pass a
// reason and it is worded consistently, pass none and the gain is silent (used for the
// state-change refills, where the surrounding message already explains itself).
...
// Anything sweet is worth a flat bonus on top of the normal meal, whatever it happened to
// be. Sweetness is read off the food's own SUGAR/FRUIT flags rather than checked against
// diet_list, so the rule stays honest if her diet is ever retuned - and berries count, since
// they carry FRUIT rather than SUGAR.
...
// Mending an agent with a bolt. It only feeds the meter if there was actually something
// wrong with them - a magical girl gets nothing out of "saving" somebody who was fine.
// A WHITE bolt mends the mind and needs missing SP; anything else mends the body and needs
// brute, burn or toxin. Being hurt in a way she cannot touch (oxygen, clone, stamina, or a
// wounded mind when she is firing a body-bolt) says so explicitly, because otherwise the
// meter just quietly fails to move and the player cannot tell why.
...
// Damage drains villainy. In the passive state that is POST-resistance damage with a floor,
// so chip damage is beneath her notice. While hysteric it is the RAW damage instead: her
// resistances are 95% by then, so post-resist numbers would be ~1 and beating her down -
// the intended way for the facility to force the breach - would do nothing at all.
...
/mob/living/simple_animal/hostile/limbus_abno/hatred_queen/proc/OnMobDeath(datum/source, mob/living/died, gibbed)
	...
	if(!ishuman(died) || !died.mind)
		return
	// Tell her WHY the bar just moved, or the largest passive gain in her kit is invisible.
	// She is only named the victim when she could actually have seen it - the signal fires
	// across the whole z-level, and naming someone dying three departments away would hand
	// her intelligence she has no business having.
...
// Hitting another specimen is heroism as far as she is concerned. This hooks UnarmedAttack
// rather than AttackingTarget, because AttackingTarget is the AI melee path and she is
// always player-driven.
...
// Hysteria and the marker projection both freeze the body and can overlap. Tracking the
// reasons separately means whichever clears first does not hand movement back early.
// NOTE: can_act is what actually stops a player-driven simple mob - TRAIT_IMMOBILIZED does
// nothing here, because a player walks the mob through hostile/Move(), which never reads it.
// The base gates Move() and AttackingTarget() on can_act for free; OpenFire and every
// ability proc has to check it by hand.
		...
		for(var/turf/TF in hit_line)
			for(var/mob/living/L in range(beam_stage - 1, TF))
				if(L == src || (L in already_hit) || L.stat == DEAD || (L.status_flags & GODMODE))
					continue
				already_hit += L
				if(IsFriend(L)) // Her beam mends the people she has decided to protect.
...
/mob/living/simple_animal/hostile/limbus_abno/hatred_queen/proc/BeatsTurf(turf/T, list/beats_hit)
	...
	for(var/turf/TT in range(1, T))
		...
		for(var/mob/living/L in TT)
			if(L == src || (L in beats_hit) || L.stat == DEAD || (L.status_flags & GODMODE) || IsFriend(L))
				continue
			beats_hit += L
...
// Can she reach for the marker right now? The passive form needs to be either convinced the
// world still needs her OR genuinely content - either bar over its threshold will do. The
// dragon has no such problem: it is past caring what anyone needs.
/mob/living/simple_animal/hostile/limbus_abno/hatred_queen/proc/CanUseMarker()
	// The spawn lockout comes FIRST, ahead of the breach bypass: it is about the start of
	// the round, not about what she currently is. In practice the two never meet - the
	// villainy bar cannot empty inside five minutes - but the ordering says which wins.
...
// Two presses. The first sends her out as a target reticle only she can see; her body is
// left standing, visible and vulnerable, and cannot move or fight. The second brings her
// home, and three seconds later the body phases to where the marker stood.
...
// The dragon does not simply arrive. Copied down from the source's TeleportExplode, at a
// smaller radius and much lower damage - this is a repositioning tool with teeth, not a
// nuke, and she can throw it as often as the action's cooldown allows.
// Friends are still spared, the same way her beam and Arcana Beats spare them; if a breached
// Queen should ignore her old friend list entirely, drop the IsFriend check here.
...
// Sending the marker out. The press that brings her home is a separate innate action that
// lives on the marker itself (below) - the LCL action base binds itself to a limbus abno on
// Grant, so it cannot be handed to a camera mob.
/datum/action/cooldown/limbus_abno_action/qoh_marker
	name = "Arcane Marker"
	...
	button_icon = 'ModularLobotomy/_Lobotomyicons/lcl_abno_actions.dmi'
	background_icon_state = "bg_qoh"
	// NOT actions_abnormality.dmi's "qoh_teleport": that is a dark purple silhouette and it
	// disappears against bg_qoh's dark interior - it reads as a button with no icon at all.
	// The bespoke reticle is the same motif the marker itself uses, and is actually visible.
