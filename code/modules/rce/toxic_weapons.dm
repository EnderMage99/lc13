...
/obj/item/ego_weapon/ranged/acid_sprayer/afterattack(atom/target, mob/living/user, proximity_flag, params)
	...
	for(var/turf/T in affected_turfs)
		...
		for(var/mob/living/L in T)
			...
			L.apply_damage(damage_amount * damage_mult, TOX)
			L.apply_damage(damage_amount * 0.5 * damage_mult, FIRE)
			// Apply a venom stack
...
/obj/projectile/venom_shell/on_hit(atom/target, blocked)
	...
	if(isliving(target))
		...
		if(L.has_status_effect(/datum/status_effect/venom_stacks))
			...
			L.apply_damage(bonus_damage, TOX)
...
/obj/effect/moving_toxic_cloud/process()
	...
	for(var/turf/T in range(2, src))
		for(var/mob/living/L in T)
			...
			L.apply_damage(damage_per_tick * damage_mult, TOX)
			L.apply_status_effect(/datum/status_effect/venom_stacks)
...
/obj/structure/venom_spike_strip/proc/trigger(mob/living/victim)
	...
	victim.apply_damage(damage, BRUTE, pick(BODY_ZONE_L_LEG, BODY_ZONE_R_LEG))
	for(var/i = 1 to venom_stacks)
		victim.apply_status_effect(/datum/status_effect/venom_stacks)
...
/obj/item/ego_weapon/plague_scythe/proc/spin_damage(mob/user, spin_number)
	...
	for(var/mob/living/L in range(reach, user))
		...
		L.apply_damage(30 + venom_bonus, BRUTE)
		L.apply_damage(20 + venom_bonus, TOX)
...
/datum/status_effect/acid_decay/tick()
	owner.apply_damage(damage_per_tick, TOX)
	// Visual feedback
	if(prob(30))
...
/datum/status_effect/venom_stacks/on_creation(mob/living/new_owner, stacks_to_add = 1)
	...
	if(stacks > 1)
		to_chat(new_owner, span_danger("Venom stacks increased to [stacks]!"))

/datum/status_effect/venom_stacks/tick()
	owner.apply_damage(damage_per_tick * stacks, TOX)
	// Chance for additional effects at high stacks
	if(stacks >= 5 && prob(20))
		owner.Dizzy(20)
	if(stacks >= 8 && prob(10))
		owner.Paralyze(10)

/datum/status_effect/venom_stacks/refresh(effect)
	var/datum/status_effect/venom_stacks/V = effect
	var/old_stacks = stacks
	stacks = min(stacks + V.stacks, max_stacks)
	if(stacks > old_stacks)
		to_chat(owner, span_danger("Venom stacks increased to [stacks]!"))
	if(stacks == max_stacks)
		to_chat(owner, span_userdanger("Maximum venom saturation reached!"))
...
// Visual effect for venom marking
...
/obj/effect/temp_visual/venom_mark/Initialize()
	. = ..()
	color = "#00FF00"
	animate(src, alpha = 0, transform = matrix() * 2, time = duration)

// Venom explosion visual
...
/obj/effect/temp_visual/venom_explosion/Initialize()
	. = ..()
	color = "#00FF00"
